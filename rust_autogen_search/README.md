...
Single-thread exhaustive Sheffer auto-generation checker used in the EML
pipeline.
...
- Defaults are moderate, but this tool can become expensive for larger
  `--gen-k`, `--verify-k`, or very large generation languages.
- The run configuration is printed at startup in a `Run config:` line.
...
exercising profile-0 unary/binary search.

Expected key output lines:

- `Run config: gen_k=3 ... verify_k=4 ...`
