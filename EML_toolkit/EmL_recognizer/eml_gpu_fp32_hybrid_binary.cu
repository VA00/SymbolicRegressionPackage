...
static int parse_target_value(const char* expr, double* out)
{
    char buf[256];
...
int main(int argc, char** argv)
{
    ...
    for (int i = 1; i < argc; ++i) {
        if (strcmp(argv[i], "--help") == 0 || strcmp(argv[i], "-h") == 0) {
            ...
            allow_inf = 1;
        } else if (strcmp(argv[i], "--disable-inf") == 0 ||
                   strcmp(argv[i], "--no-allow-inf") == 0) {
            allow_inf = 0;
        } else if (argv[i][0] != '-') {
            target_expr = argv[i];
        } else {
    ...
    if (!parse_target_value(target_expr, &target_double)) {
        fprintf(stderr, "Could not parse numeric target value: %s\n", target_expr);
        return 1;
    }
    ...
    printf("Max tokens:          %d\n", max_tokens);
    printf("Semantics:           %s\n", allow_inf ? "extended-real" : "finite-real (--disable-inf)");
