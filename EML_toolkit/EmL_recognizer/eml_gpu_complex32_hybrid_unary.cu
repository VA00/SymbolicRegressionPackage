...
__constant__ complex32 d_symbol_value_f;

static unsigned long long h_shape_count[MAX_LEAVES + 1];
static const complex64 h_symbol_value = complex64(SYMBOL_VALUE, 0.0);
...
{
    if (leaves == 1) {
        return (leaf_mask & 1U) ? h_symbol_value : complex64(1.0, 0.0);
    }
    ...
    complex64 left_value = evaluate_expr_complex64(left_leaves, left_rank, left_mask);
    complex64 right_value = evaluate_expr_complex64(right_leaves, right_rank, right_mask);
...
)
{
    ...
    while (sp > 0) {
        ...
        if (current_leaves == 1) {
            int bit = (leaf_mask >> leaf_pos) & 1U;
            values[vsp++] = bit ? d_symbol_value_f : complex32(1.0f, 0.0f);
            leaf_pos++;
            sp--;
            continue;
        }
...
)
{
    ...
    complex32 computed = evaluate_expr_complex32(leaves, shape_idx, leaf_mask);
...
static void print_usage(const char* argv0)
{
    ...
    printf("\n");
    printf("Leaves are drawn from {1, %s}; targets are numeric complex pairs RE,IM.\n", SYMBOL_NAME);
    printf("Complex arithmetic uses the principal branch of log.\n");
    printf("\n");
    printf("Examples:\n");
    printf("  %s --target 0,1 --max-tokens 31\n", prog);
    printf("  %s --target -0.57721566490153286060651209008240,0 --max-tokens 31\n", prog);
...
int main(int argc, char** argv)
{
    ...
    complex32 symbol_value_f((float)h_symbol_value.real(), (float)h_symbol_value.imag());
    ...
    printf("=== EML Complex Unary Search (complex32 GPU + complex64 CPU verification) ===\n");
    ...
    for (int K = 1; K <= max_tokens; K += 2) {
        ...
        for (unsigned long long offset = 0; offset < total_expr; offset += chunk_size) {
            ...
            search_eml_unary_kernel<<<blocks, DEFAULT_THREADS>>>(
                leaves, count, offset, assignment_count, assignment_mask, target_value_f,
                candidate_threshold, d_candidates, d_candidate_count, max_candidates
            );
            ...
            for (int i = 0; i < copied_candidates; ++i) {
                complex64 value = evaluate_expr_complex64(
                    leaves, h_candidates[i].shape_idx, h_candidates[i].leaf_mask
                );
