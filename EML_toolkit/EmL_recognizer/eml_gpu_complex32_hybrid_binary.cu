...
__constant__ unsigned long long d_assignment_count[MAX_LEAVES + 1];
__constant__ complex32 d_leaf_values_f[3];

static unsigned long long h_shape_count[MAX_LEAVES + 1];
static unsigned long long h_assignment_count[MAX_LEAVES + 1];
static const complex64 h_leaf_values[3] = {
    complex64(1.0, 0.0),
    complex64(SYMBOL1_VALUE, 0.0),
    complex64(SYMBOL2_VALUE, 0.0)
};
...
static complex64 evaluate_expr_complex64(
    int leaves,
...
)
{
    ...
    complex64 left_value = evaluate_expr_complex64(left_leaves, left_rank, left_code);
    complex64 right_value = evaluate_expr_complex64(leaves - left_leaves, right_rank, right_code);
...
__device__ complex32 evaluate_expr_complex32(
    int leaves,
    unsigned long long shape_rank,
    unsigned long long leaf_code
)
{
    ...
    unsigned long long assignment_base[MAX_LEAVES];
    ...
    while (sp > 0) {
        ...
        if (current_leaves == 1) {
            int digit = (int)(assignment_base[top] % 3ULL);
        ...
        if (stage[top] == 0) {
            ...
            unsigned long long left_base = d_assignment_count[left_leaves];
            assignment_base[sp - 1] = assignment_base[top] % left_base;
            assignment_base[top] /= left_base;
            continue;
        }
        ...
        if (stage[top] == 1) {
            ...
            stage[sp] = 0;
            assignment_base[sp] = assignment_base[top];
            sp++;
            continue;
        }
...
)
{
    ...
    complex32 computed = evaluate_expr_complex32(leaves, shape_idx, leaf_code);
...
static void print_usage(const char* argv0)
{
    ...
    printf("\n");
    printf("Leaves are drawn from {1, %s, %s}; targets are numeric complex pairs RE,IM.\n",
           SYMBOL1_NAME, SYMBOL2_NAME);
    printf("Complex arithmetic uses the principal branch of log.\n");
    printf("\n");
    printf("Examples:\n");
    printf("  %s --target 0,1 --max-tokens 31\n", prog);
    printf("  %s --target 0.57721566490153286060651209008240,0 --max-tokens 1\n", prog);
    printf("  %s --target 0.9505961575598952,0 --max-tokens 25\n", prog);
}
...
int main(int argc, char** argv)
{
    ...
    complex32 leaf_values_f[3];
    for (int i = 0; i < 3; ++i) {
        leaf_values_f[i] = complex32((float)h_leaf_values[i].real(), (float)h_leaf_values[i].imag());
    }

    init_counts();
    CUDA_CHECK(cudaMemcpyToSymbol(d_shape_count, h_shape_count, sizeof(h_shape_count)));
    CUDA_CHECK(cudaMemcpyToSymbol(d_assignment_count, h_assignment_count, sizeof(h_assignment_count)));
    CUDA_CHECK(cudaMemcpyToSymbol(d_leaf_values_f, leaf_values_f, sizeof(leaf_values_f)));

    cudaDeviceProp prop;
    CUDA_CHECK(cudaGetDeviceProperties(&prop, 0));

    printf("=== EML Complex Binary Search (complex32 GPU + complex64 CPU verification) ===\n");
    ...
    for (int K = 1; K <= max_tokens; K += 2) {
        ...
        for (unsigned long long offset = 0; offset < total_expr; offset += chunk_size) {
            ...
            search_eml_binary_kernel<<<blocks, DEFAULT_THREADS>>>(
                leaves, count, offset, assignment_count, target_value_f, candidate_threshold,
            ...
            for (int i = 0; i < copied_candidates; ++i) {
                complex64 value = evaluate_expr_complex64(
                    leaves, h_candidates[i].shape_idx, h_candidates[i].leaf_code
                );
