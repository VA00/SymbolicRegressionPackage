...
twoEML[]:=EML[EML[1,EML[EML[1,1],1]],EML[EML[EML[1,EML[EML[1,EML[1,EML[1,1]]],1]],EML[EML[1,1],1]],1]]
expEML[x_] := EML[x, 1];
...
sqrtEML[x_] := expEML[halfEML[logEML[x]]];
powerEML[x_, y_] := expEML[timesEML[y, logEML[x]]];
logaritmEML[base_, x_] := divideEML[logEML[x], logEML[base]];
hypotEML[x_, y_] := sqrtEML[plusEML[sqrEML[x], sqrEML[y]]];
logisticSigmoidEML[x_] := invEML[EML[minusEML[x], expEML[-1]]];
coshEML[x_] := avgEML[expEML[x], expEML[minusEML[x]]];
sinhEML[x_] := EML[x, expEML[coshEML[x]]]
...
sqrtW[x_] := Exp[Log[x]/2];
powerW[x_, y_] := Exp[y Log[x]];
logarithmW[base_, x_] := Log[x]/Log[base];
...
checkIdentity[name_String, lhs_, rhs_, vars_List, extraAssumptions_: True] := Module[
  ...
  Print["  N[LHS,64]: ", Check[N[lhs, 64], $Failed]];
  Print["  N[RHS,64]: ", Check[N[rhs, 64], $Failed]];
...
result = Catch[
  (
    ...
    checkIdentity["Constant 0", zeroEML[], 0, {}];
    checkIdentity["Constant -1", negEML[], -1, {}];
    checkIdentity["Constant 2", twoEML[], 2, {}];
    ...
    *)
    ...
    checkIdentity["Sqrt[x] witness for x>0", sqrtW[x], Sqrt[x], {x}, x > 0];
    checkIdentity["Power[x,y] witness for x>0", powerW[x, y], x^y, {x, y}, x > 0];
    checkIdentity["Log[b,x] witness for b>0,b!=1,x>0", logarithmW[b, x], Log[b, x], {b, x}, b > 0 && b != 1 && x > 0];
