...
def main() -> int:
    ...
    for expr, name, xmin, xmax, step in CASES:
        run(["python3", "./make_eml_numpy.py", expr, name, str(xmin), str(xmax), str(step)])
        out = parse_output(run(["python3", "./test_eml_numpy.py"]))
    ...
    report = "\n".join(lines) + "\n"
    path = DIR / "unary_suite_report_numpy.txt"
