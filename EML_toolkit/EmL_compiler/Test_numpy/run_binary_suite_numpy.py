...
def main() -> int:
    ...
    for expr, xmin, xmax, xstep, ymin, ymax, ystep in CASES:
        ...
        out = parse_output(run([sys.executable, "./test_eml_binary_numpy.py"]))
    ...
    lines = ["Binary EML Suite: numpy", "", f"Testing on [{grid_xmin:g}, {grid_xmax:g}, {grid_step:g}]^2 grid", ""]
    for r in results:
    ...
    report = "\n".join(lines) + "\n"
    path = DIR / "binary_suite_report_numpy.txt"
