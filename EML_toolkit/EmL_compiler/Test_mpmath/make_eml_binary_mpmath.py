...
def build_parser() -> argparse.ArgumentParser:
    return argparse.ArgumentParser(
        description="Generate an mpmath test for one Wolfram-style binary expression.",
        usage="%(prog)s F[x,y] [xMin xMax dx yMin yMax dy [digits]]",
        epilog=(
            "Examples:\n"
...
def parse_ranges(ap: argparse.ArgumentParser, bounds: list[str]) -> tuple[str, str, str, str, str, str, str]:
    x_min = "-4.0"
    x_max = "4.0"
    x_step = "0.5"
    y_min = "-4.0"
    ...
    y_step = "0.5"
    digits = "64"
    if len(bounds) not in (0, 6, 7):
        ap.error("expected either no bounds, 6 numbers, or 6 numbers plus digits")
    if len(bounds) >= 6:
        x_min, x_max, x_step, y_min, y_max, y_step = bounds[:6]
    if len(bounds) == 7:
        digits = bounds[6]
    return x_min, x_max, x_step, y_min, y_max, y_step, digits
...
def main() -> int:
    ...
    text = (DIR / "test_eml_binary_mpmath.py.in").read_text(encoding="utf-8")
    repl = {
        "@NAME@": FUNC_NAME,
        "@REF@": ref,
        "@REF_LABEL@": ref_label,
    ...
    out = DIR / "test_eml_binary_mpmath.py"
