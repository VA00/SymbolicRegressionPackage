...
fn main() {
    ...
    let effective_rayon_threads = rayon::current_num_threads();
    ...
    if run.p0 {
        ...
        let unary_candidates: Vec<&Cand> = unary_pool.iter().take(profile0_unary_take).collect();
        let binary_candidates: Vec<&Cand> = binary_pool.iter().take(profile0_binary_take).collect();
        let p0_hits: Vec<(String, String, String)> = unary_candidates
            .par_iter()
            .map(|u| {
                ...
                for b in &binary_candidates {
                    ...
                    if let Some(name) = satisfies_any_family(
                        &constants,
                        &unary,
                        &binary,
                        &[],
                        ...
                        local_hits.push((name.to_string(), u.expr.clone(), b.expr.clone()));
                    }
                }
                local_hits
            })
        ...
        let mut p0_hits_sorted = p0_hits;
        p0_hits_sorted.sort();
        for (name, unary_expr, binary_expr) in &p0_hits_sorted {
            println!(
                "HIT[{name}] unary={} | binary={}",
                fmt_mma_expr(unary_expr),
                fmt_mma_expr(binary_expr)
            );
        }
        hits_p0 += p0_hits_sorted.len();
        hits += p0_hits_sorted.len();
